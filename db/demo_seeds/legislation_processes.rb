section "Creating DEMO collaborative legislation" do
  ...
  process.documents.create!(title: "Air quality regulation", attachment: document, user_id: User.first.id)

  document = File.new(Rails.root.join("db", "demo_seeds", "documents", "legislation", "processes", "pollution-information-report.pdf"))
  process.documents.create!(title: "Pollution information report", attachment: document, user_id: User.first.id)
  ...
  process.draft_versions.create!(title: "Proposal for a new Ordinance regulating the holding and protection of animals.",
                                         body: body,
                                         created_at: 10.days.ago,
